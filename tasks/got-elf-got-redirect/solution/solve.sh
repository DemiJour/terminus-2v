#!/usr/bin/env bash
# Oracle must be self-contained: Harbor mounts only this script at /oracle/.
# Source of truth for the embedded Go: patcher.go (kept in-repo for editing).
set -euo pipefail
cat > /app/cmd/got-patch/main.go <<'__GOTPATCH_MAIN__'
package main
...
__GOTPATCH_MAIN__
cd /app
go build -o /app/got-patch ./cmd/got-patch
