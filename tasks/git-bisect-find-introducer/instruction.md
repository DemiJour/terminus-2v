...
Somewhere in history, someone added a single line to `src/main.py` that is **exactly** `BUG_INTRODUCED` (whole line, nothing else on that line).
...
Sort `files_touched`, list each path once. Set `bug_line_is_exact` to true only if `src/main.py` at that commit actually contains a line that is exactly `BUG_INTRODUCED`.
